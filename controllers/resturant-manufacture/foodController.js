...
const updateFood = asyncHandler(async (req, res, next) => {
  const { id } = req.params;
  const { image, productName, productPrice, description, productIncludes } =
    req.body;
  const food = await Food.findById(id);
  ...
  if (food.owner.toString() !== req.user._id.toString()) {
    return next(
      new ErrorResponse(`You have no permission to update this product`, 403)
    );
  }
...
const deleteFood = asyncHandler(async (req, res, next) => {
  ...
  if (food.owner.toString() !== req.user._id.toString()) {
    return next(
      new ErrorResponse(`You have no permission to delete this product`, 403)
    );
  }
