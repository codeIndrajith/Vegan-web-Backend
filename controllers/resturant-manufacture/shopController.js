...
const addShop = asyncHandler(async (req, res, next) => {
    const { image, shopName, description, openHours, locations, services } = req.body;
...
const updateShop = asyncHandler(async(req, res, next) => {
    ...
    if(shop.owner.toString() !== req.user._id.toString()) {
        return next(new ErrorResponse(`You have no permission to update this shop`,403))
    }
...
const deleteShop = asyncHandler(async(req, res, next) => {
    ...
    if(shop.owner.toString() !== req.user._id.toString()) {
        return next(new ErrorResponse(`You have no permission to delete this shop`,403))
    }
